...
app_name   = "AnimeScaler"
version    = "v1"
githubme   = "https://github.com/Djdefrag/AnimeScaler/releases"
...
OUTPUT_PATH_CODED    = "Разместить результат в исходной папке"
...
def check_upscale_steps() -> None:
    ...
    while True:
        ...
        if actual_step == COMPLETED_STATUS:
            ...
            info_message.set(f"Обработка апскейла прекращена")
            stop_upscale_process()
            place_upscale_button()
            break
...
def open_files_action():
    ...
    if supported_files_counter > 0:
        ...
        info_message.set("Неподдерживаемые файлы :(")
...
def place_loadFile_section():
    ...
    text_drop = (" Список поддерживаемых форматов \n\n "
               + "ФОТОГРАФИИ • jpg, png, tif, bmp, webp, heic \n " 
...
def place_AI_menu():
    ...
    info_button = create_info_button(open_info_AI_model, "AI модель")
...
def place_AI_blending_menu():
    ...
    info_button = create_info_button(open_info_AI_blending, "AI смешивание")
...
def place_AI_multithreading_menu():
    ...
    info_button = create_info_button(open_info_AI_multithreading, "AI многопоточность")
...
def place_input_output_resolution_textboxs():
    ...
    # Input scale %
    info_button = create_info_button(open_info_input_resolution, "Масш исход изобр %")
    ...
    # Output scale %
    info_button = create_info_button(open_info_output_resolution, "Масш выходн изобр %")
